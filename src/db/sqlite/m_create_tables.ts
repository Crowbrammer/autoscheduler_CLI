...
export class CreateTablesMigration {
    ...
    async up(loud?: boolean): Promise<void> {
        ...
        await this.driver.query('CREATE TABLE actions (id INTEGER PRIMARY KEY AUTO_INCREMENT, name VARCHAR(255), duration INTEGER, date_created DATETIME DEFAULT (NOW()), date_updated DATETIME DEFAULT (NOW()));');
        if (loud) console.log('Migrating for schedule_templates');
        await this.driver.query('CREATE TABLE schedule_templates (id INTEGER PRIMARY KEY AUTO_INCREMENT, name VARCHAR(255), is_current BOOLEAN, date_created DATETIME DEFAULT (NOW()), date_last_used DATETIME DEFAULT (NOW()) );');
        if (loud) console.log('Migrating for purposes');
        await this.driver.query('CREATE TABLE purposes (id INTEGER PRIMARY KEY AUTO_INCREMENT, name VARCHAR(255), is_current BOOLEAN, date_created DATETIME DEFAULT (NOW()), date_last_used DATETIME DEFAULT (NOW()) );');
        if (loud) console.log('Migrating for outcomes');
        await this.driver.query('CREATE TABLE outcomes (id INTEGER PRIMARY KEY AUTO_INCREMENT, name VARCHAR(255), is_current BOOLEAN, date_created DATETIME DEFAULT (NOW()), date_last_used DATETIME DEFAULT (NOW()) );');
        if (loud) console.log('Migrating for obstacles');
        await this.driver.query('CREATE TABLE obstacles (id INTEGER PRIMARY KEY AUTO_INCREMENT, name VARCHAR(255), is_current BOOLEAN, is_handled BOOLEAN, is_deleted BOOLEAN, date_created DATETIME DEFAULT (NOW()), date_last_used DATETIME DEFAULT (NOW()) );');
        if (loud) console.log('Migrating for decisions');
        await this.driver.query('CREATE TABLE decisions (id INTEGER PRIMARY KEY AUTO_INCREMENT, name VARCHAR(255), is_current BOOLEAN, date_created DATETIME DEFAULT (NOW()), date_last_used DATETIME DEFAULT (NOW()) );');
        if (loud) console.log('Migrating for schedules');
        await this.driver.query('CREATE TABLE schedules (id INTEGER PRIMARY KEY AUTO_INCREMENT, name VARCHAR(255), based_on_template_id INTEGER, is_current BOOLEAN, date_created DATETIME DEFAULT (NOW()), date_last_used DATETIME DEFAULT (NOW()) );');
        if (loud) console.log('Migrating for events');
        await this.driver.query('CREATE TABLE events (id INTEGER PRIMARY KEY AUTO_INCREMENT, summary VARCHAR(255), start DATETIME, end DATETIME, base_action_id INTEGER, date_created DATETIME DEFAULT (NOW()), date_last_used DATETIME DEFAULT (NOW()) );');
...
async function main(): Promise<void> {
    ...
    db.query = db.all;
    const migration = new CreateTablesMigration({driver: db});
