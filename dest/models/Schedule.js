...
const EventBuilder_1 = require("../builders/EventBuilder");
...
class Schedule extends Model_1.AutoschedulerModel {
    ...
    async buildEvents() {
        ...
        this.events = [await EventBuilder_1.default.create({ action: as[0], start: this.start })];
        // Build the rest of the this.events in order, using the previous event's end time as the start.
        for (let i = 1; i < as.length; i++) {
            const action = as[i];
            this.events.push(await EventBuilder_1.default.create({ action, start: this.events[i - 1].end }));
        }
    ...
    async getEvents() {
        ...
        return pulledEvents.map(e => new Event_1.default(e));
    }
    async isCurrent() {
        if (!this.id)
    ...
    async save(isCurrent) {
        // Add all the events
        let scheduleId;
        if (isCurrent)
            await Model_1.AutoschedulerModel.driver.query('UPDATE schedules SET is_current = false;');
        if (Model_1.AutoschedulerModel.driver.constructor.name === 'PQuery') {
            scheduleId = (await Model_1.AutoschedulerModel.driver.query(`INSERT INTO schedules (name, based_on_template_id, is_current) VALUES ('${this.name}', ${this.templateId}, ${isCurrent ? true : false});`)).insertId;
        }
        else {
            scheduleId = (await Model_1.AutoschedulerModel.driver.query(`INSERT INTO schedules (name, based_on_template_id, is_current) VALUES ('${this.name}', ${this.templateId}, ${isCurrent ? true : false});`)).lastID;
        }
        for (let i = 0; i < this.events.length; i++) {
            const event = this.events[i];
            let eventId;
